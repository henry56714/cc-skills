...
4. 沿关键链做有针对性的读取或导航。先保证至少能走完 6 跳；接口分派、回调、权限/变体或异步链仍未到终端时可扩到 12 次读取。不要为了省读取停在中间节点；达到 12 次仍无法判断时进入 `needs_review`，不要静默丢弃。
...
6. 为每个输出记录建立结构化 `root_cause`。它描述**需要一次修复的根因**，不是当前候选的命中位置：
   ...
   Hunter 的 `root_cause_hint` 只用于把疑似重复项放在同批；必须独立核实，不能照抄为结论。
